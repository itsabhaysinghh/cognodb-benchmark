...
class CognoDBAdapter(BaseDatabaseAdapter):

    def __init__(self, name: str = "CognoDB Cloud", config: Optional[Dict[str, Any]] = None):
    ...
    def connect(self) -> None:
        if not self.is_configured():
            raise ValueError("CognoDB credentials missing from environment.")
        if not self.driver:
